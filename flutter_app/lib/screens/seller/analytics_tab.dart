...
class AnalyticsTab extends StatefulWidget {
  ...
  const AnalyticsTab({super.key, required this.onExit});
  @override
  State<AnalyticsTab> createState() => _AnalyticsTabState();
}

class _AnalyticsTabState extends State<AnalyticsTab> {
  late Future<dynamic> future = Api.instance.get('/api/seller/analytics');
  int period = 7;
  bool pdfBusy = false;
      ...
      Row(children: [
        for (var i = 0; i < days.length; i++)
          Expanded(child: Builder(builder: (_) {
            ...
            final show = wide || i % 5 == 0 || i == days.length - 1;
            if (!show) return const SizedBox.shrink();
            return Column(children: [
              if (wide) Text(week[dt.weekday % 7], style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: isToday ? p.success : p.muted, height: 1.1)),
              Text('${dt.day}', style: TextStyle(fontSize: 10, color: isToday ? p.success : p.muted.withValues(alpha: .8), height: 1.1))
            ]);
          })),
      ]),
      const SizedBox(height: 8),
      Row(children: [_legend(p.accent, 'Buyurtmalar'), const SizedBox(width: 14), _legend(p.success, 'Bugun')]),
    ]);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: RefreshIndicator(
        onRefresh: () async => setState(() => future = Api.instance.get('/api/seller/analytics')),
        child: FutureBuilder(
          future: future,
          initialData: Api.instance.cached('/api/seller/analytics'),
          builder: (_, snap) {
            if (!snap.hasData)
              return ListView(padding: EdgeInsets.zero, children: [
                SellerHeader('Analitika', onExit: widget.onExit),
                const Padding(
                    padding: EdgeInsets.fromLTRB(20, 18, 20, 0),
                    child: Column(children: [Skeleton(height: 70), SizedBox(height: 10), Skeleton(height: 70), SizedBox(height: 10), Skeleton(height: 240)]))
              ]);
            final d = snap.data;
            final s = d['stats'];
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  ...
                  if (tm != null) ...[
                    DarkBanner(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text('Bu oy sotuv · ${monthName('${tm['month']}')}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFFB7C0CC))),
                        ...
                        Row(children: [
                          Expanded(child: _monthBox(const Color(0xFF7EE2B0), '${n(tm['done'])}', 'Sotildi · ${pct(n(tm['done']), n(tm['total']))}%', onDark: true)),
                          const SizedBox(width: 8),
                          Expanded(child: _monthBox(const Color(0xFFFF8A8E), '${n(tm['cancelled'])}', 'Bekor · ${pct(n(tm['cancelled']), n(tm['total']))}%', onDark: true)),
                          const SizedBox(width: 8),
                          Expanded(child: _monthBox(const Color(0xFFFFD166), '${n(tm['new'])}', 'Kutilmoqda', onDark: true)),
                        ]),
                      ]),
                    ),
                  ...
                  // ---- 6 ta ko'rsatkich ----
                  Row(children: [
                    Expanded(child: stat(Icons.receipt_long_outlined, p.accent, p.accentSoft, '${n(s['newOrders'])}', 'Yangi buyurtma')),
                    const SizedBox(width: 10),
                    Expanded(child: stat(Icons.account_balance_wallet_outlined, p.success, p.successSoft, fmtPrice(s['revenue'] ?? 0), "Sotuv, so'm"))
                  ]),
                  const SizedBox(height: 10),
                  Row(children: [
                    Expanded(child: stat(Icons.person_outline_rounded, violet, violetSoft, '${n(s['customers'])}', 'Xaridorlar')),
                    const SizedBox(width: 10),
                    Expanded(child: stat(Icons.shopping_bag_outlined, violet, violetSoft, '${n(s['unitsSold'])}', 'Sotilgan dona'))
                  ]),
                  const SizedBox(height: 10),
                  Row(children: [
                    Expanded(child: stat(Icons.account_balance_wallet_outlined, amber, amberSoft, fmtPrice(s['avgCheck'] ?? 0), "O'rtacha chek")),
                    const SizedBox(width: 10),
                    Expanded(child: stat(Icons.visibility_outlined, amber, amberSoft, fmtPrice(s['views'] ?? 0), "Ko'rish · ${s['conversion'] ?? 0}% konv."))
                  ]),
                  ...
                  card(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        Expanded(
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          const Text('Buyurtmalar dinamikasi', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
                          Text(
                      ...
                      const SizedBox(height: 12),
                      ChoiceChips(items: const [('7', '7 kun'), ('30', '30 kun')], value: '$period', onChanged: (v) => setState(() => period = int.parse(v))),
                      const SizedBox(height: 14),
                      if (daysN == 0)
                        Padding(
                            padding: const EdgeInsets.symmetric(vertical: 18),
                            child: Center(child: Text("Bu davrda buyurtma bo'lmadi — do'kon havolasini ulashing", textAlign: TextAlign.center, style: TextStyle(color: p.muted, fontSize: 13))))
                      else
                        chart(days),
                    ]),
                  ),
                  ...
                  if (customers.isNotEmpty)
                    section(
                        ...
                        'Eng faol xaridorlar',
                        listCard([
                          for (var i = 0; i < customers.take(3).length; i++)
                            listItem(
                              leading: Container(
                                  width: 36,
                                  height: 36,
                            ...
                            if (d['peakHour'] != null || d['bestWeekday'] != null)
                              Padding(
                                  ...
                                  child: Text.rich(TextSpan(style: TextStyle(fontSize: 12, color: p.muted), children: [
                                    if (d['peakHour'] != null) ...[
                                      const TextSpan(text: 'Eng faol soat: '),
                                      TextSpan(
                                    ...
                                    if (d['bestWeekday'] != null) ...[
                                      const TextSpan(text: 'Eng sotuvli kun: '),
                                  ...
                                  Expanded(
                                      child: Padding(
                                          ...
                                          child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [
                                            Text(n(w['n']) > 0 ? '${n(w['n'])}' : '', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w800)),
                                            Container(
                                            ...
                                            const SizedBox(height: 4),
                                            Text('${w['label']}'.substring(0, 2), style: TextStyle(fontSize: 10, color: p.muted, fontWeight: FontWeight.w700)),
                                          ]))),
                              ]),
                            ),
                          ]),
                  ...
                  card(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                      Row(children: [
                        ...
                        Expanded(
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          const Text('PDF hisobot', style: TextStyle(fontWeight: FontWeight.w800)),
                          Text("Barcha ko'rsatkichlar, jadvallar va tavsiyalar bitta faylda. Telegram botga yuboriladi.", style: TextStyle(fontSize: 12, color: p.muted))
                        ])),
                      ]),
                      const SizedBox(height: 10),
                      FilledButton.icon(
                          onPressed: pdfBusy ? null : downloadPdf,
                          icon: pdfBusy ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.ios_share, size: 18),
                          label: Text(pdfBusy ? 'Tayyorlanmoqda...' : 'PDF hisobotni yuklab olish')),
                    ]),
                  ),
                  if (n(s['newOrders']) > 0) ...[
                    const SizedBox(height: 10),
                    OutlinedButton.icon(
                        ...
                        label: Text("${n(s['newOrders'])} ta yangi buyurtmani ko'rish"),
                        onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => OrdersTab(onExit: widget.onExit)))),
                  ],
                ]),
              ),
            ]);
...
class _LinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const top = 18.0, bottom = 10.0;
    final h = size.height - top - bottom;
    final max = values.fold<int>(1, (m, v) => v > m ? v : m);
    final step = values.length == 1 ? 0.0 : size.width / (values.length - 1);
    final pts = [for (var i = 0; i < values.length; i++) Offset(values.length == 1 ? size.width / 2 : i * step, top + h - h * values[i] / max)];
...
/// PDF hisobot: Telegram botga yuboriladi, bo'lmasa yuklab olish havolasi ochiladi
Future<bool> downloadReport(BuildContext context) async {
  try {
    final r = await Api.instance.post('/api/seller/report');
    if (r['sent'] == true) {
      if (context.mounted) showToast(context, 'PDF hisobot Telegram botga yuborildi');
    } else {
      await launchUrl(Uri.parse('$apiBase${r['url']}'), mode: LaunchMode.externalApplication);
    }
    return true;
  } catch (e) {

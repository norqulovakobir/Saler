...
class _ReelsScreenState extends State<ReelsScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    // Reels tasdiqlangan hisob talab qiladi: mehmon uchun so'rov bo'lim ochilganda yuboriladi (ilova yuklanganda emas)
    if (_visible || Api.instance.registered) {
      load();
    } else {
      loading = false;
    }
  ...
  void _onTab() {
    // Bo'lim birinchi marta ochilganda yuklanadi (mehmonga tasdiqlash oynasi chiqadi)
    if (_visible && items.isEmpty && !loading && error == null) load();
    _visible ? _startTiming() : _sendView();
  }
  ...
  List<_Reel> _parse(dynamic r) => ((r is Map ? r['items'] as List? : r as List?) ?? const []).map((e) => _Reel.fromJson(e as Map)).toList();
